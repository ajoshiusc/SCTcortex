function [ testStats fit ] = hypothesis_test_hist_regression_all (histsSqrtCrossExp_all, hists, cogScore)
...
numOfSubjects = max ( size (histsSqrtCrossExp_all, 2), size (hists, 2) );
...
for subsetNum = 1 : numOfSubsets
    ...
    if numel (histsSqrtCrossExp_all > 0)
        [ valuesPredicted_part, predictionErrors_part, predictionRMSE_part, conc_part ] ...
            = kernel_regression_hyperSphere_all (histsSqrtCrossExp_all, indicesVertex, cogScore);
    else
        [ valuesPredicted_part, predictionErrors_part, predictionRMSE_part ] ...
            = linear_regression_all (hists, indicesVertex, cogScore);
    end
