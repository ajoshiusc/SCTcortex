...
numOfMeshNodes = length(tar.vertices);
...
SCTgrp1 = SCTgrp1 (:,:,1:count1); SCTgrp1 = permute (SCTgrp1, [1 3 2]);
SCTgrp2 = SCTgrp2 (:,:,1:count2); SCTgrp2 = permute (SCTgrp2, [1 3 2]);
...
for nb=1:20
  if nb==1
    ...
    [ t2unpermuted t2permuted_all ] = permutation_test(datasample(SCTgrp1(6:15,:,:),size(SCTgrp1,2),2), datasample(SCTgrp2(6:15,:,:),size(SCTgrp2,2),2), numOfPermutations);
  end
...
export_fig -a1 -m2 -transparent Pics/bootstrap_sim_SCT_right.png, close
...
% see p values
show_surf_val(tarsm,(1-pValUnpermuted1),[0,1],cmap);
